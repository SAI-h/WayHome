<template>
    <div class="formPage">
        <el-row>
            <el-col :span="21">
                <h4 style="margin-left: 70px;">新增线路信息表</h4>
            </el-col>
            <el-col :span="3">
        ...
        <el-form-item prop="busNum" label="线路号" 
            :rules="{ required: true, message: '请输入线路号', trigger: 'blur' }">
            <el-input v-model="dynamicValidateForm.busNum" style="width:80%" placeholder="请填入公交线路号"></el-input>
        </el-form-item>
        <el-form-item
            v-for="(point, index) in dynamicValidateForm.points"
            :label="labelFormatter(index)"
            :key="point.key"
            :prop="'points.' + index + '.staLng'"
            ...
            <el-row :gutter="30" v-if="dynamicValidateForm.points[index].staID===-1">
                <el-col :span="5">
                    <el-form-item class="lnglat">
                        <el-input v-model="dynamicValidateForm.points[index].staLng" :disabled="true" placeholder="请点击地图"></el-input>
                    </el-form-item>
                </el-col>
                <el-col :span="5">
                    <el-form-item class="lnglat">
                        <el-input v-model="dynamicValidateForm.points[index].staLat" :disabled="true" placeholder="请点击地图"></el-input>
                    </el-form-item>
                </el-col>
                <el-col :span="14">
        ...
        <!-- <el-form-item label="全程长度">
            <h5 style="float: left; color: gray;">{{ Math.round(dynamicValidateForm.totalLen) }}米</h5>
        </el-form-item> -->
        <el-form-item>
            <el-button style="width: 100%; margin-left: -80px;" type="warning" plain size="small" @click="submitForm('dynamicValidateForm')">提交</el-button>
        </el-form-item>
        </el-form>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        submitForm(formName) { // 表单提交
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    MessageBox.confirm("是否提交当前公交路线的相关信息", "提示信息", {
                        confirmButtonText: '确定',
                        cancelButtonText: '取消',
                    }).then(() => {
                        let args = {
                            routeName: this.dynamicValidateForm.busNum,
                            cityID: JSON.parse(sessionStorage.getItem('city')).id,
                            editTime: moment(new Date()).format('YYYY-MM-DD HH:mm:ss'),
                            points: []
                        };
                        for(let point of this.dynamicValidateForm.points) {
                            if(point.staID === CONTROL_POINT) {
                                args.points.push({
                                    staID: null,
                                    pointID: null,
                                    pointLng: point.staLng,
                                    pointLat: point.staLat
                                });
                            }
                            else {
                                args.points.push({
                        ...
                        // let bus = this.dynamicValidateForm.busNum;
                        // let now = moment(new Date()).format('YYYY-MM-DD HH:mm:ss');
                        // let cityID = JSON.parse(sessionStorage.getItem('city')).id;
                        // for(let point of arr) {
                        //     if(point.staID === CONTROL_POINT) {
                        //         // busNum, staID, isStation, pointLng, pointLat, editTime, cityID
                        //         args.push([bus, null, 0, point.staLng, point.staLat, now, cityID]);
                        //     }
                        //     else {
                        //         // busNum, staID, isStation, pointLng, pointLat, editTime, cityID
        ...
        getLocation_2(location) { // 获取到来自地图对应位置的坐标值,并将其存入对应points的index索引下
            if(this.controlID !== -1) {
                this.dynamicValidateForm.points[this.controlID].staLng = location.lng;
                this.dynamicValidateForm.points[this.controlID].staLat = location.lat;
            }
        },
        ...
        getTotalLength(points) { // 获取公交路线的总长度
            ...
            for(let i = 0; i < points.length - 1; i ++) {
                let first = new AMap.LngLat(points[i].LngLat[0], points[i].LngLat[1]);
                let second = new AMap.LngLat(points[i + 1].LngLat[0], points[i + 1].LngLat[1]);
    ...
    computed: { // 计算属性
        controlBan() { // 确保每次仅能对一个控制点进行设置
            let arr = this.dynamicValidateForm.points;
            let cnt = 0;
            for(let i = 0; i < arr.length; i ++) cnt += arr[i].staID === CONTROL_POINT && arr[i].staLng === '';
            return cnt === 1;
        }
    },
    watch: {
            ...
            handler() {
                ...
                for(let point of pArr) {
                    if(point.staID === CONTROL_POINT) { // 若是控制点
                        if(point.staLng !== '' && point.staLat !== '') { // 该控制点已经完成了设置
                            drawPoints.push(
                                {
                                    isStation: false, // 标志为控制点
                                    LngLat: [point.staLng, point.staLat]
                                }
                            );
                        }
                    }
                ...
                // this.dynamicValidateForm.totalLen = this.getTotalLength(drawPoints);
            }
        }
    }
}
